package net.yundingwei.dszw.app.common.widgets.button;
...
public class SwitchButton extends CompoundButton {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.SwitchButton_switch_thumb_drawable);
            thumbColor = ta.getColorStateList(R.styleable.SwitchButton_switch_thumb_color);
            margin = ta.getDimension(R.styleable.SwitchButton_switch_thumb_margin, margin);
            marginLeft = ta.getDimension(R.styleable.SwitchButton_switch_thumb_marginLeft, margin);
            marginRight = ta.getDimension(R.styleable.SwitchButton_switch_thumb_marginRight, margin);
            marginTop = ta.getDimension(R.styleable.SwitchButton_switch_thumb_marginTop, margin);
            marginBottom = ta.getDimension(R.styleable.SwitchButton_switch_thumb_marginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.SwitchButton_switch_thumb_width, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.SwitchButton_switch_thumb_height, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.SwitchButton_switch_thumb_radius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchButton_switch_back_radius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.SwitchButton_switch_back_drawable);
            backColor = ta.getColorStateList(R.styleable.SwitchButton_switch_back_color);
            backMeasureRatio = ta.getFloat(R.styleable.SwitchButton_switch_back_measure_ratio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchButton_switch_animation_duration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.SwitchButton_switch_fade_back, true);
            tintColor = ta.getColor(R.styleable.SwitchButton_switch_tint_color, tintColor);
            textOn = ta.getString(R.styleable.SwitchButton_switch_text_on);
            textOff = ta.getString(R.styleable.SwitchButton_switch_text_off);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.SwitchButton_switch_text_marginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.SwitchButton_switch_autoAdjustTextPosition, autoAdjustTextPosition);
            ta.recycle();
        }
    ...
    private Drawable zoomDrawable(Drawable drawable, int w, int h) {
        ...
        return new BitmapDrawable(getResources(), newbmp);       // 把 bitmap 转换成 drawable 并返回
    }
